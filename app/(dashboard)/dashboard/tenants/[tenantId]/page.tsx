...
import { getTenant, type Tenant } from "@/lib/tenants-api";
import { useAuth } from "@/providers/auth-provider";
...
export default function TenantDashboardPage() {
  ...
  useEffect(() => {
    if (authLoading || !accessToken || !tenantId) {
      return;
    }
  ...
  }, [accessToken, authLoading, tenantId]);

  /* ------------------------------------------------------------------------ */
  /* Load tenant authorization                                                */
  /* ------------------------------------------------------------------------ */

  useEffect(() => {
    if (authLoading || !accessToken || !user?.id || !tenantId) {
      return;
    }

    void loadTenantAuthorization(tenantId).catch((error) => {
      console.error("[TENANT] Failed to load tenant authorization:", error);
    });
  ...
  if (authLoading || tenantLoading || tenantAuthorizationLoading) {
    return (
    ...
    return (
      <main className="identity-page flex min-h-screen items-center justify-center p-6">
        <div className="identity-surface w-full max-w-md rounded-2xl p-8 text-center">
          <h1 className="text-xl font-semibold">Tenant unavailable</h1>
  ...
  const isTenantMember = tenantAuthorization?.isMember === true;

  const canManageTenant = isSuperAdmin || isTenantMember;

  if (!canManageTenant) {
    return (
      <main className="identity-page flex min-h-screen items-center justify-center p-6">
        <div className="identity-surface w-full max-w-md rounded-2xl p-8 text-center">
          <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-[var(--danger-soft)] text-xl text-[var(--danger)]">
            !
          </div>
          ...
          <p className="identity-muted mt-2 text-sm">
            You are not a member of this tenant and do not have access to its
            administration.
          </p>
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-8">
          ...
          <div className="mt-5">
            ...
            <p className="identity-muted mt-1 text-sm">
              Manage this tenant and its members, roles, and permissions.
            </p>
          </div>
        </div>

        {/* Tenant information */}
        <div className="identity-surface mb-8 rounded-2xl p-6">
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            <TenantInfo label="Name" value={tenant.name} />

            <TenantInfo label="Slug" value={tenant.slug} />

            <TenantInfo
              label="Status"
              value={tenant.isActive ? "ACTIVE" : "INACTIVE"}
            />
            <TenantInfo label="Tenant ID" value={tenant.id} mono />
          </div>
        </div>

        {/* Management */}
        <div className="mb-3">
          <p className="identity-eyebrow">Tenant management</p>
        </div>
        ...
        <div className="grid gap-5 md:grid-cols-3">
          <TenantManagementCard
            ...
            title="Members"
            description="View members, manage membership, and assign tenant roles."
          />

          <TenantManagementCard
            href={`/dashboard/tenants/${tenantId}/roles`}
            title="Roles"
            description="Create and manage roles belonging to this tenant."
          />

          <TenantManagementCard
            href={`/dashboard/tenants/${tenantId}/permissions`}
            title="Permissions"
            description="View and manage permissions available to this tenant."
          />
        </div>

        {/* Authorization information */}
        {tenantAuthorization && (
          <div className="identity-surface mt-8 rounded-2xl p-6">
            <p className="identity-eyebrow">Your tenant access</p>

            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              <div>
                <p className="identity-muted text-xs">Membership</p>

                <p className="mt-1 text-sm font-medium">
                  {isTenantMember ? "Member" : "Not a member"}
                </p>
              </div>

              <div>
                <p className="identity-muted text-xs">Roles</p>

                <p className="mt-1 text-sm font-medium">
                  {tenantAuthorization.roles?.length || 0}
                </p>
              </div>

              <div>
                <p className="identity-muted text-xs">Permissions</p>
...
function TenantInfo({
  ...
  return (
    <div>
      <p className="identity-eyebrow">{label}</p>
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="identity-muted mt-2 text-sm leading-6">{description}</p>
        </div>
